...
עודכן: 2026-07-26 | commit: 87a920d (בסיס קוד: 5d3a6cd)
...
ממשק עברית RTL בלבד. כלי פרטי לשני בני הזוג בלבד — אין תוכנית מוצר לקהל רחב (אישור הבעלים).
...
| CI / בדיקות אוטומטיות בענן | לא קיים (מכוון) | אין `.github/workflows`; הרצת בדיקות ידנית לפני commit (אישור הבעלים) |
| פריסת הגשר אצל שני בני הזוג | עובד | האפליקציה מתפקדת אצל שניהם (אישור הבעלים) |
...
## 10. שאלות פתוחות
1. **פתוחה — לבחינה בהמשך**: האם לפרוס את Tailwind ו-Tesseract.js כ-self-hosted (כמו שנעשה ל-pdf.js),
   או להשאיר את התלות ב-CDN? נוגע ישירות לחוב 9.1 (אופליין + content blockers).

נענו (אישור הבעלים, 2026-07-26) — לא לשאול שוב:
2. הגשר פרוס והאפליקציה מתפקדת אצל שני בני הזוג. (האם הפריסה היא בדיוק v1.4.0 — לא ידוע.)
3. המשתמשים היחידים הם שני בני הזוג; אין כוונה להפוך את זה למוצר לאחרים כרגע.
4. אין CI — כל הבדיקות (`node tests/smoke.mjs`, `node --check`) מורצות ידנית לפני commit.
5. `docs/design-prompt.md` עדיין מחייב — העיצוב הנוכחי תואם לו.

## שינויים מאז העדכון הקודם
- סעיף 10: 4 מתוך 5 השאלות נענו ע"י הבעלים; נותרה פתוחה רק שאלת ה-CDN מול self-hosting (9.1).
- סעיף 1: הובהר שהאפליקציה היא כלי פרטי לשני בני הזוג, ללא תוכנית מוצר.
- סעיף 8: נוספה שורת "פריסת הגשר" (עובד), ושורת ה-CI סומנה כהיעדר מכוון ולא כפער.
- סעיף 7/9: ההסתמכות על בדיקות ידניות אושרה כבחירה, לא כליקוי — חוב 9.7 נשאר רלוונטי לכיסוי בלבד.
