...
- **פרטיות**: כספת הדרכונים (`vault`) לא עוזבת את המכשיר לעולם — לא לסנכרון, לא לגיבוי, לא ל-Gemini. חריג יחיד: כשמעלים דרכון כמסמך, הצילום נשלח פעם אחת ל-Gemini לחילוץ פרטים (יצירת בן משפחה אוטומטית), ואז עובר לכספת ונמחק מהמסמכים. מסמכי `category==='passport'` לעולם לא מועלים לדרייב (דילוג ב-Sync). מפתחות API וכתובות גשר מקומיים למכשיר.
