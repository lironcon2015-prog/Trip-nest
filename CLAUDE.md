...
- **זיכרון הסוכן — שלוש שכבות**: (1) זיכרון משפחה — פתקי `agentNotes` בלי `tripId`, מוזרקים תמיד; (2) זיכרון טיול — פתקים עם `tripId` בתוך הקונטקסט של טיולים נוכחיים/עתידיים, וסיכומי `agentTripSummaries` (נוצרים אוטומטית בסוף טיול או ידנית ממסך הזיכרון 🧠) כשדה `memory` של טיולי עבר; (3) ארכיון שיחות מלא (`js/archive.js`) — תורות שנחתכים מחלון ה-40 נשמרים ב-IndexedDB (store `archive`) וממוראים ל-`chat-archive-<tripId|general>.json` בדרייב (מיזוג union לפי id; דורש גשר v1.3.0+, אחרת מקומי בלבד). הסוכן ניגש לארכיון רק דרך הכלי `search_archive` (ללא אישור משתמש — קריאה בלבד). `agentHistory`/`agentNotes`/`agentTripSummaries` הם `SHARED_SETTINGS`. תורות היסטוריה נושאים `_ts`/`_trip` — חובה להסיר (`wire()`) לפני שליחה ל-Gemini.
